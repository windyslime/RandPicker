...
import conf
import update
from app_utils.ui_components.history_card import HistoryCard
from app_utils.ui_components.update_confirm_box import UpdateConfirmBox
from app_utils.ui_components.group_card import GroupCard
from app_utils.ui_components.group_edit_box import GroupEditBox
from app_utils.ui_components.group_enable_policy_box import GroupEnablePolicyBox
...
try:
    if os.path.exists(HISTORY_FILE):
        with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
            raw_historys = json.load(f)
            for item in raw_historys:
                try:
                    # Ensure student is a dictionary with at least name and id
                    student_data = item.get('student', {})
                    if not isinstance(student_data, dict) or \
                        'name' not in student_data or 'id' not in student_data:
                        logger.warning(f"Skipping history item due to missing/invalid student data: {item}")
                        continue

                    historys.append({
                        "mode": item["mode"],
                        "student": {"name": student_data["name"], "id": student_data["id"]}, # Ensure only name and id are used
                        "time": datetime.fromisoformat(item["time"])
                    })
                except (TypeError, ValueError, KeyError) as e:
                    logger.warning(f"Skipping history item due to parsing error: {item}, Error: {e}")
        logger.info(f"Successfully loaded {len(historys)} entries from {HISTORY_FILE}")
    else:
        logger.info(f"{HISTORY_FILE} not found. Starting with empty history.")
except json.JSONDecodeError:
    logger.error(f"Failed to decode {HISTORY_FILE}. Starting with empty history.", exc_info=True)
    historys = []  # Ensure historys is reset if file is corrupt
except Exception as e:
    logger.error(f"An unexpected error occurred while loading history from {HISTORY_FILE}: {e}", exc_info=True)
    historys = [] # Ensure historys is reset on other errors
...
class Settings(FluentWindow):
    ...
    def setup_history_interface(self):
        ...
        self.btnClearHistory = self.historyInterface.findChild(PushButton, "btnClearHistory")
        if self.btnClearHistory: # Check if button is found
            self.btnClearHistory.setIcon(fIcon.DELETE) # Optional: set an icon
            self.btnClearHistory.clicked.connect(self.confirm_clear_history)
        else:
            logger.warning("Could not find btnClearHistory in history.ui")
    ...
    def confirm_clear_history(self):
        ...
        msgBox.yesButton.setText("确认清除")
        msgBox.cancelButton.setText("取消")
        if msgBox.exec():
            self.execute_clear_history()

    def execute_clear_history(self):
        global historys # Ensure we're modifying the global list
        historys.clear()
        ...
        Flyout.create(
            icon=InfoBarIcon.SUCCESS,
            title="历史记录已清除",
            content="所有历史记录已被成功移除。",
            target=self.btnClearHistory, # Target the button itself
            parent=self.historyInterface, # Parent to the history tab
            isClosable=True,
            aniType=FlyoutAnimationType.PULL_UP,
        )
